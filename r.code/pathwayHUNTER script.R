...
PathwayHUNTER <- function(x,y,z,num){
  ...
  tidycluster_unite <- tidy_ClustersFilter %>%
    tidyr::unite(word, word1, word2, sep = " ")} else{
  ...
  Themes2.0<- Themes %>% dplyr::group_by(cluster) %>% dplyr::mutate(word = paste0(word, collapse = " \\\ ")) %>%
  ...
  p <- dplyr::select(myData, -c(cluster,level,n,tf,idf,tf_idf,word,GOID))
  ...
  default.hmap <- ComplexHeatmap::Heatmap(pp,cluster_row_slices = FALSE,row_title_gp = gpar(font = .0001),
                                          row_title_rot = 0,row_gap = unit( 4, "mm"),split = split)
  ...
  Table <- dplyr::select(myData, -c(level,n,tf,idf,tf_idf))
